                    ...
                    if data_str:
                        ...
                        if len(data_parts) >= 33:
                            results[code_key] = {
                                ...
                                'change_pct': (float(data_parts[3]) - float(data_parts[2])) / float(data_parts[2]) * 100 if float(data_parts[2]) > 0 else 0,
                            }
...
def get_sina_hist_data(stock_code):
    """获取新浪财经历史数据用于计算技术指标"""
    try:
        # 判断交易所
        ...
        # 获取近90日K线数据
        url = f"https://quotes.sina.cn/cn/api/quotes.php?symbol={symbol}&datasource=quotes&day=90&_={int(time.time()*1000)}"
        resp = requests.get(url, timeout=10)
        data = resp.json()
        
        if 'result' in data and 'data' in data['result']:
            klines = data['result']['data']
...
def analyze_stock(stock_name, stock_code):
    """分析单只股票技术指标"""
    df = get_sina_hist_data(stock_code)
